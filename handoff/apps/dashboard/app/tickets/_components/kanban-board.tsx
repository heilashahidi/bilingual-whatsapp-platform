...
export function KanbanBoard({
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      ...
      <DragOverlay>
        {activeTicket ? (
          <CardContent ticket={activeTicket} userById={userById} dragging />
        ) : null}
      </DragOverlay>
...
function DraggableCard({
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="relative">
      ...
      >
        <CardContent ticket={ticket} userById={userById} />
      </Link>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="mt-2.5 flex items-center gap-2">
        <span
          className={`h-2 w-2 rounded-full ${connDot}`}
          title={`Connectivity: ${ticket.agent.connectivityStatus}`}
        />
